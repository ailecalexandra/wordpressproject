...
class Products_Renderer extends \WC_Shortcode_Products {
	...
	public function __construct( $settings = [], $type = 'products' ) {
        ...
        add_action( 'woocommerce_after_shop_loop_item_title', array($this,'get_description_of_products'), 7 );
	}

    public function get_description_of_products() {
        echo '<p class="twbb_woocommerce-loop-product__desc">' . $this->get_woocommerce_excerpt() . '</p>';
    }
    protected function get_woocommerce_excerpt(){
        $excerpt = get_the_excerpt();
        $excerpt = strip_shortcodes($excerpt);
        $excerpt = strip_tags($excerpt);
        $excerpt = trim($excerpt);
        $excerpt_length = isset( $this->settings['description_length'] ) ? $this->settings['description_length']: 25;
