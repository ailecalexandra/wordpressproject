...
class Facebook_SDK_Manager {
  ...
  public function register_admin_fields( Settings $settings ) {
    $settings->add_section(Settings::TAB_INTEGRATIONS, 'facebook_sdk', [
      'callback' => function() {
        ...
        echo sprintf(__('Facebook SDK lets you connect to your <a href="%s" target="_blank">dedicated application</a> so you can track the Facebook Widgets analytics on your site.', 'tenweb-builder'), 'https://developers.facebook.com/docs/apps/register/') . '<br>' . '<br>' . __('If you are using the Facebook Comments Widget, you can add moderating options through your application. Note that this option will not work on local sites and on domains that don\'t have public access.', 'tenweb-builder');
      },
      'fields' => [
        'pro_facebook_app_id' => [
  ...
  private function validate_sdk() {
    ...
    if ( !empty($errors) ) {
      wp_die($message, __('Facebook SDK', 'tenweb-builder'), [ 'back_link' => TRUE ]);
    }
  }
}
