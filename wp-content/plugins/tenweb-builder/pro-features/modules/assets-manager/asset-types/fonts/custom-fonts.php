<?php
namespace Tenweb_Builder\ElementorPro\Modules\AssetsManager\AssetTypes\Fonts;

use Elementor\Core\Files\Assets\Files_Upload_Handler;
use Elementor\Core\Files\CSS\Base;
use Tenweb_Builder\ElementorPro\Modules\AssetsManager\Classes;
use Tenweb_Builder\ElementorPro\Modules\AssetsManager\AssetTypes\Fonts_Manager;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	public function wp_handle_upload_prefilter( $file ) {
		 ...
		 */
		$svg_handler = \Elementor\Plugin::instance()->assets_manager->get_asset( 'svg-handler' );
	...
	public function get_font_face_from_data( $font_family, $data ) {
		$src = [];
		foreach ( [ 'eot', 'woff2', 'woff', 'ttf', 'svg', 'otf' ] as $type ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'elementor_tenweb/custom_fonts/font_display', 'auto', $font_family, $data ) . ';' . PHP_EOL;
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
            ...
            'otf' => esc_html__( 'OpenType Fonts, Developed by Microsoft and Adobe', 'elementor-pro' ),
		];
