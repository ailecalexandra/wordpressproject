...
class Form_Record {
	...
	public function validate( $ajax_handler ) {
      ...
      /**
       * 10Web
       * Additionally check if field sanitation is ok to show a correct message.
       */
      if ( '' === $field['value'] && '' !== $field['raw_value'] ) {
        $ajax_handler->add_error( $id, Ajax_Handler::get_default_message( Ajax_Handler::INVALID_FIELD, $this->form_settings ) );
      }
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "elementor_tenweb/forms/validation/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'elementor_tenweb/forms/validation', $this, $ajax_handler );
	...
	public function process_fields( $ajax_handler ) {
		foreach ( $this->fields as $id => $field ) {
			 ...
			 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
			 */
			do_action( "elementor_tenweb/forms/process/{$field_type}", $field, $this, $ajax_handler );
		}
		 ...
		 * @param Ajax_Handler $ajax_handler An instance of the ajax handler.
		 */
		do_action( 'elementor_tenweb/forms/process', $this, $ajax_handler );
	}
	...
	private function sanitize_field( $field, $value ) {
		...
		switch ( $field_type ) {
			...
			default:
				 ...
				 */
				$value = apply_filters( "elementor_tenweb/forms/sanitize/{$field_type}", $value, $field );
		}
