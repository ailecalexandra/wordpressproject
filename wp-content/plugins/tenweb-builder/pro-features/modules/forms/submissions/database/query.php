<?php
namespace Tenweb_Builder\ElementorPro\Modules\Forms\Submissions\Database;
...
class Query extends Base_Object {
	...
	public function add_submission( array $submission_data, array $fields_data ) {
		...
		try {
			...
			\Elementor\Plugin::instance()->logger->get_logger()->error( 'Save submission failed, db error: ' . $e->getMessage() );
		}
	...
	private function get_submission_body( $submission, $with_form_fields = false ) {
		...
		$document = \Elementor\Plugin::instance()->documents->get( $edit_post_id );
